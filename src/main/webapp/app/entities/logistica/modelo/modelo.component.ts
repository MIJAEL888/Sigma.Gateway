...
export class ModeloComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.modeloService
      ...
      .subscribe(
        (res: HttpResponse<IModelo[]>) => this.paginateModelos(res.body, res.headers),
  ...
  registerChangeInModelos() {
    this.eventSubscriber = this.eventManager.subscribe('modeloListModification', response => this.reset());
  }
  ...
  protected paginateModelos(data: IModelo[], headers: HttpHeaders) {
